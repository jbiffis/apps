...
- **Frontend:** React 19 + Vite · Tailwind · `vite-plugin-pwa` (installable PWA; the service worker is intentionally network-only / no-cache for now so changes show up immediately — only the icons + manifest are precached)
